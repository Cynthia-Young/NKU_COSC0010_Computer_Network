...
bool SendMsg(char* data, SOCKET sockClient, SOCKADDR_IN addrSrv, int dataLen, char fileNum)
{
	...
	while (head <= pcknum - 1)
	{
		...
		if (tail - head + 1 < (int)cwnd && tail != pcknum - 1) //如果没超过窗口大小，且没发到结尾
		{
			// 设置信息头
			...
			SendPkg(p, sockClient, addrSrv);
			tail++;
			printf("%s [ SEND ] Client: WINDOW SIZE: %d, SSTHRESH: %d, WINDOW HEAD: %d, WINDOW TAIL: %d, TOTAL NUM: %d\n", timei(), (int)floor(cwnd), ssthresh, head, tail, pcknum);
		...
		if (timer_list.size() != 0)
		{
			...
			{
				tail = head - 1;
				ssthresh = (int)max((cwnd / 2), 1);
				cwnd = 1;
				dupACKcount = 0;
				printf("%s [ ERR  ] Client: wait for a long time error\n", timei());
				printf("%s [ INFO ] Client: WINDOW SIZE: %d, SSTHRESH: %d, WINDOW HEAD: %d, WINDOW TAIL: %d, TOTAL NUM: %d\n", timei(), (int)floor(cwnd), ssthresh, head, tail, pcknum);
	...
	while (1)
	{
		if (recvfrom(sockClient, (char*)&p1, sizeof(p1), 0, (SOCKADDR*)&addrSrv, &addrlen) > 0)
		{
			...
			if (p1.hm.type != ACK && ck != 1)  //类型错误或校验和有误
			{
				...
				printf("%s [ INFO ] Client: WINDOW SIZE: %d, SSTHRESH: %d, WINDOW HEAD: %d, WINDOW TAIL: %d\n", timei(), (int)floor(cwnd), ssthresh, head, tail);
				continue;
			}
			else
			{
				...
				else if (head % 256 > 256 - floor(cwnd) && p1.hm.seq < floor(cwnd) - (256 - head % 256))
				{
